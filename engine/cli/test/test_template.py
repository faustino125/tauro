...
- Input validation (project names, sandboxes)
...
from engine.cli.core import ConfigFormat, ExitCode
...
class TestTemplateError:
    ...
    def test_template_error_inherits_from_engine_error(self):
        """Test TemplateError inherits from engineError."""
        from engine.cli.core import engineError
...
class TestBaseTemplate:
    ...
    def test_template_get_file_extension_yaml(self):
        ...
        assert template.get_file_extension() == ".yaml"
    ...
    def test_template_get_file_extension_json(self):
        ...
        assert template.get_file_extension() == ".json"
    ...
    def test_template_get_file_extension_dsl(self):
        ...
        assert template.get_file_extension() == ".dsl"
...
class TestMedallionBasicTemplate:
    ...
    def test_generate_global_settings(self):
        ...
        assert "csv" in settings["supported_formats"]
    ...
    def test_generate_pipelines_config(self):
        ...
        assert "ml_train" in pipelines
        assert "ml_predict" in pipelines
        assert "streaming" in pipelines
    ...
    def test_nodes_have_required_fields(self):
        ...
        for node_name, node_config in nodes.items():
            ...
            assert "dependencies" in node_config
...
class TestTemplateFactory:
    ...
    def test_create_template_invalid_type(self):
        """Test creating template with invalid type raises error."""
        fake_type = TemplateType.MEDALLION_BASIC  # We only have one type
        # This won't fail with the current implementation as there's only one
        # But test the error message structure
        template = TemplateFactory.create_template(fake_type, "test")
        assert isinstance(template, MedallionBasicTemplate)
...
class TestTemplateGeneratorValidation:
    """Tests for TemplateGenerator input validation."""

    def test_valid_project_name(self):
        """Test that valid project names pass validation."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir) / "valid_project")
            # Should not raise
            generator._validate_inputs("valid_project")

    def test_project_name_with_hyphens(self):
        """Test project name with hyphens is valid."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir) / "my-project")
            generator._validate_inputs("my-project")

    def test_project_name_with_underscores(self):
        """Test project name with underscores is valid."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir) / "my_project")
            generator._validate_inputs("my_project")

    def test_invalid_project_name_empty(self):
        """Test that empty project name raises error."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir))
            with pytest.raises(TemplateError) as exc_info:
                generator._validate_inputs("")
            assert "non-empty string" in str(exc_info.value)

    def test_invalid_project_name_special_chars(self):
        """Test that special characters in project name raise error."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir))
            with pytest.raises(TemplateError) as exc_info:
                generator._validate_inputs("invalid@project!")
            assert "invalid characters" in str(exc_info.value)

    def test_invalid_project_name_spaces(self):
        """Test that spaces in project name raise error."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir))
            with pytest.raises(TemplateError) as exc_info:
                generator._validate_inputs("my project")
    ...
    def test_project_name_too_long(self):
        """Test that very long project names raise error."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir))
            long_name = "a" * 300
            with pytest.raises(TemplateError) as exc_info:
                generator._validate_inputs(long_name)
            assert "between 1 and 255" in str(exc_info.value)

    def test_valid_developer_sandboxes(self):
        """Test valid developer sandbox names."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir))
            generator._validate_inputs("my_project", ["alice", "bob"])

    def test_invalid_developer_sandbox_special_chars(self):
        """Test that special characters in sandbox names raise error."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir))
            with pytest.raises(TemplateError) as exc_info:
                generator._validate_inputs("my_project", ["alice@domain"])
            assert "invalid characters" in str(exc_info.value)

    def test_invalid_developer_sandboxes_not_list(self):
        """Test that non-list developer_sandboxes raises error."""
        with tempfile.TemporaryDirectory() as tmpdir:
            generator = TemplateGenerator(Path(tmpdir))
            with pytest.raises(TemplateError) as exc_info:
                generator._validate_inputs("my_project", "alice")
...
class TestTemplateGeneratorExecution:
    ...
    def test_generate_project_with_sample_code(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert (output_path / "pipelines" / "ml_features.py").exists()
            assert (output_path / "pipelines" / "ml_train.py").exists()
            assert (output_path / "pipelines" / "ml_predict.py").exists()
            assert (output_path / "pipelines" / "streaming.py").exists()
    ...
    def test_generate_project_creates_requirements(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check requirements contain engine
            content = req_path.read_text()
            assert "engine" in content

    def test_generate_project_invalid_name(self):
    ...
    def test_generate_project_without_yaml_library(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            import engine.cli.template as template_module
