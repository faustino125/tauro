...
from engine.cli.config import AppConfigManager
from engine.cli.core import ConfigurationError
...
def test_app_config_manager_loads_and_merges_envs(tmp_path: Path, monkeypatch):
    ...
    from engine.cli.core import SecurityValidator
    ...
    # Las rutas son absolutas después de _validate_and_build_paths
    assert (
        "config/dev/input.json" in envs["input_config_path"].replace("\\", "/")
        or "config\\dev\\input.json" in envs["input_config_path"]
    )
