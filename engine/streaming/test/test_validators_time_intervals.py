...
from engine.streaming.exceptions import StreamingValidationError
from engine.streaming.constants import StreamingFormat
...
class TestTimeIntervalValidation:
    ...
    def test_edge_case_very_large_number(self):
        ...
        # Should pass if <= 1 year in seconds
        assert self.validator._validate_time_interval("99999 seconds") is True  # ~1.16 days
        # 9999999 milliseconds = 9999.999 seconds = ~2.77 hours (within limit)
        assert self.validator._validate_time_interval("9999999 milliseconds") is True
        # Test exceeding 1 year limit
        assert self.validator._validate_time_interval("32000000 seconds") is False  # > 1 year
