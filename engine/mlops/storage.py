...
from engine.mlops.validators import PathValidator
...
class DatabricksStorageBackend(StorageBackend):
    ...
    def __init__(self, catalog: str, schema: str, workspace_url: str = None, token: str = None):
        ...
        Warning:
            This backend is currently PARTIALLY IMPLEMENTED.

            ✅ Working: Metadata operations (stores paths/URIs)
            ⚠️  Limited: Read operations (raise NotImplementedError with guidance)
            📝 Recommended: Use Spark DataFrame API directly for read/write operations

            For production Databricks workflows:
            - Metadata tracking: Use this backend (works)
            - Data read/write: Use spark.table() and df.write.saveAsTable()
            - Artifacts: Use UC Volumes with dbfs:/Volumes/{catalog}/{schema}/path

            This allows MLOps tracking while using Spark for actual data operations.
        """
        ...
        # Log warning about partial implementation
        logger.warning(
            f"DatabricksStorageBackend initialized for {catalog}.{schema}. "
            f"This backend is PARTIALLY IMPLEMENTED. "
            f"Read operations will raise NotImplementedError with guidance. "
            f"For production, use Spark DataFrame API directly for data operations."
        )

        if not self.workspace_url or not self.token:
            logger.warning(
                "Databricks workspace_url and token not provided. "
                "Some operations may fail. Set DATABRICKS_HOST and DATABRICKS_TOKEN env vars."
            )
    ...
    def read_dataframe(self, path: str) -> pd.DataFrame:
        ...
        Note:
            This operation is not implemented in the storage backend.
            Use Spark DataFrame API directly in your Databricks notebook:

            >>> df = spark.table(f"{self.catalog}.{self.schema}.{path}")
            >>> pandas_df = df.toPandas()  # If you need pandas
        """
        raise NotImplementedError(
            f"Reading DataFrames from Databricks is not implemented in storage backend.\n"
            f"Use Spark DataFrame API directly in your notebook:\n"
            f"  df = spark.table('{self.catalog}.{self.schema}.{path}')\n"
            f"  # Or to pandas: df.toPandas()"
        )
    ...
    def read_json(self, path: str) -> Dict[str, Any]:
        ...
        Note:
            This operation is not implemented in the storage backend.
            Use Databricks file system APIs or dbutils in your notebook:

            >>> import json
            >>> content = dbutils.fs.head(f"dbfs:/Volumes/{self.catalog}/{self.schema}/{path}")
            >>> data = json.loads(content)
        """
        raise NotImplementedError(
            f"Reading JSON from Databricks is not implemented in storage backend.\n"
            f"Use dbutils or Databricks file APIs in your notebook:\n"
            f"  content = dbutils.fs.head('dbfs:/Volumes/{self.catalog}/{self.schema}/{path}')\n"
            f"  data = json.loads(content)"
        )
    ...
    def read_artifact(self, path: str, local_destination: str) -> None:
        ...
        Note:
            This operation is not implemented in the storage backend.
            Use dbutils to copy from UC Volumes:

            >>> dbutils.fs.cp(f"dbfs:/Volumes/{self.catalog}/{self.schema}/{path}",
            ...               f"file:{local_destination}", recurse=True)
        """
        raise NotImplementedError(
            f"Downloading artifacts from Databricks is not implemented in storage backend.\n"
            f"Use dbutils in your notebook:\n"
            f"  dbutils.fs.cp('dbfs:/Volumes/{self.catalog}/{self.schema}/{path}', \n"
            f"                'file:{local_destination}', recurse=True)"
        )
