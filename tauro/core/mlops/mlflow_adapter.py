...
class MLflowPipelineTracker:
        ...
        try:
            ...
            try:
                ...
                if experiment:
                    ...
                    logger.info(f"Using existing experiment: {self.experiment_name} (ID: {self._experiment_id})")
                else:
                    ...
                    logger.info(f"Created new experiment: {self.experiment_name} (ID: {self._experiment_id})")
            except Exception:
    ...
    def start_pipeline_run(
        ...
        try:
            ...
            with mlflow.start_run(
                ...
                logger.info(f"Started MLflow pipeline run: {run_name} (ID: {self._active_pipeline_run})")
            ...
            if node_name in self._active_node_runs:
                try:
                    ...
                    self._client.set_terminated(
                        self._active_node_runs[node_name],
                        status="FAILED"
                    )
                except Exception:
                    pass
    ...
    def from_context(context, experiment_name: Optional[str] = None) -> "MLflowPipelineTracker":
        ...
        experiment_name = experiment_name or mlflow_config.get("experiment_name") or getattr(
            context, "project_name", "tauro_pipeline"
        )
