"""
Input validation utilities for MLOps layer.
...
- Invalid metric/parameter values
"""
...
class PathValidator:
    ...
    def validate_is_file_or_dir(path: Path) -> Path:
        ...
        if not path.is_file() and not path.is_dir():
            raise ValidationError(
                f"Path must be file or directory: {path}"
            )
...
class NameValidator:
    ...
    def validate_name(name: str, entity_type: str = "entity") -> str:
        ...
        if not name[0].isalnum():
            raise ValidationError(
                f"{entity_type} name '{name}' must start with letter or number"
            )
    ...
    def validate_metric_value(key: str, value: Any) -> float:
        ...
        if not isinstance(value, (int, float)):
            raise InvalidMetricError(
                key, value,
                f"Metrics must be numeric (int or float), got {type(value).__name__}"
            )
...
class ParameterValidator:
    ...
    def _validate_key(key: Any) -> str:
        ...
        if not isinstance(key, str):
            raise InvalidParameterError(
                key,
                f"Parameter name must be string, got {type(key).__name__}"
            )
        if not key.strip():
            raise InvalidParameterError(
                key,
    ...
    def _validate_value(key: str, value: Any) -> Any:
        ...
        if isinstance(value, float):
            ...
            if math.isinf(value):
                logger.warning(
                    f"Parameter '{key}' has infinite value"
                )
        return value
    ...
    def validate_parameters(
        ...
        if len(parameters) > max_items:
            raise InvalidParameterError(
                "parameters",
                f"Too many parameters ({len(parameters)} > {max_items})"
            )
    ...
    def validate_parameter(key: str, value: Any) -> Any:
        ...
        if type(value) not in ParameterValidator.SUPPORTED_TYPES:
            raise InvalidParameterError(
                key,
                f"Parameter value type not supported: {type(value).__name__}"
            )
...
class MetadataValidator:
    ...
    def validate_tags(
        ...
        if not isinstance(tags, dict):
            raise ValidationError(
                f"Tags must be dict or None, got {type(tags).__name__}"
            )
        
        if len(tags) > max_items:
            raise ValidationError(
                f"Too many tags ({len(tags)} > {max_items})"
            )
        ...
        for key, value in tags.items():
            ...
            if not isinstance(value, str):
                raise ValidationError(
                    f"Tag value must be string, got {type(value).__name__}"
                )
    ...
    def validate_description(description: Optional[str]) -> Optional[str]:
        ...
        if len(description) > 10000:
            raise ValidationError(
                f"Description too long ({len(description)} > 10000)"
            )
...
class FrameworkValidator:
    ...
    def validate_framework(framework: str, allow_custom: bool = True) -> str:
        ...
        if not isinstance(framework, str):
            raise ValidationError(
                f"Framework must be string, got {type(framework).__name__}"
            )
