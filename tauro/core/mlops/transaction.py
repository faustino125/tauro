"""
Transaction support for MLOps operations.

Provides atomic transaction support with rollback capabilities
for operations that involve multiple storage writes.

Example:
    >>> txn = Transaction(storage, "/path/to/lock.lock")
    >>> txn.write_json(metadata, "metadata.json")
    >>> txn.write_dataframe(df, "index.parquet")
    >>> txn.execute()  # All or nothing
"""
...
from tauro.core.mlops.exceptions import MLOpsException
...
class Transaction:
    ...
    def write_dataframe(
        ...
        self.operations.append(Operation(
            operation_type="write_dataframe",
            path=path,
            data=df,
            mode=mode
        ))
                ...
                try:
                    if operation.operation_type == "write_dataframe":
                        self._original_values[operation.path] = (
                            self.storage.read_dataframe(operation.path)
                        )
                except Exception:
            ...
            if operation.operation_type == "write_dataframe":
                if operation.path in self._original_values:
                    try:
                        ...
                        logger.error(
                            f"Failed to rollback dataframe at {operation.path}: {e}"
                        )
